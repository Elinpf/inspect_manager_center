class StoreFile < ApplicationRecord
  ...
  def slug_candidates
    [
      [:client, :locate, :inspect_time],
      [:client, :locate, :inspect_time, :version]
    ]
  end
  ...
  def get_last_recently
    StoreFile.where('client = :client AND locate = :locate AND inspect_time = :inspect_time AND recently = :recently AND id != :id',
                    client:       client,
                    locate:       locate,
  ...
  # 返回所有同地市和巡检时间的历史版本
  def history
    StoreFile.where('client = :client AND locate = :locate AND inspect_time = :inspect_time',
                    client:       client,
                    locate:       locate,
                    inspect_time: inspect_time,
                   )
